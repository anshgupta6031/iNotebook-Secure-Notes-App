



const express = require('express')
const User = require("../models/User")
const { body, validationResult } = require('express-validator')
// const bcrypt = require('bcryptjs')
const jwt = require('jsonwebtoken')
const fetchuser = require("../middleware/fetchUser")                //  a middleware which embeds the auth-token to the request object and verifies if the user requesting for data has the right token or not.........


const router = express.Router()
const JWT_SECRET = "sjdgf765w3udckjwehrw8764"                   //  this can be any string which will be used to verify the user if he is already has a registered email or not.......



//  ROUTE 1 : creating a user using : POST "http://localhost:5000/api/auth/createuser"  No login required...

router.post('/createuser', [                                                      //  express-validator.........
    body("name", "Enter a valid name").isLength({ min: 3 }),
    body("email", "Enter a valid email").isEmail(),
    body("password", "Password must be at least 5 characters").isLength({ min: 5 }),
], async (req, res) => {

    const errors = validationResult(req)
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array(), success: false })
    }


    try {

        let user = await User.findOne({ email: req.body.email })
        if (user) {
            return res.status(400).json({ errors: "A user with this email already exists...", success: false })
        }


        // const salt = await bcrypt.genSalt(10)                               //  salt is added after the password to make it more secure.....
        // const securedPassword = await bcrypt.hash(req.body.password, salt)          //  password along with the salt is hashed to make a secured password.....

        user = await User.create({
            name: req.body.name,
            email: req.body.email,
            // password: securedPassword,
            password: req.body.password,
        })



        const data = {
            user: {
                id: user.id
            }
        }

        const authToken = jwt.sign(data, JWT_SECRET)                //  an authentication token is generated by joining the data with the secret(so that the user can be verified later) which is then sent as the response......
        res.json({ authToken: authToken, success: true })

    }

    catch (error) {
        console.error(error.message)
        res.status(500).json({ errors: "Internal Server Error", success: false })
    }
})




//  ROUTE 2 : Authenticating a user using : POST "http://localhost:5000/api/auth/login"  No login required...

router.post('/login', [
    body("email", "Enter a valid email").isEmail(),
    body("password", "Password cannot be blank").exists(),
], async (req, res) => {

    const errors = validationResult(req)
    if (!errors.isEmpty()) {
        return res.status(400).json({ errors: errors.array(), success: false })
    }


    const { email, password } = req.body

    try {

        let user = await User.findOne({ email })
        if (!user) {                                                    //  checking if email exists.....
            return res.status(400).json({ errors: "Please try to login with correct credentials", success: false })
        }


        // const passwordCompare = await bcrypt.compare(password, user.password)
        // if (!passwordCompare) {                                         //  if the email exist then checking if the password matches with the one in the database with the matched email.....
        //     return res.status(400).json({ errors: "Please try to login with correct credentials" })
        // }                                                           //  BUG HERE!!!       //  bcrypt.compare() is always returning false.......
        //  lines 39, 40, 45 and above are commented due to bug.....


        if (password != user.password) {
            return res.status(400).json({ errors: "Please try to login with correct credentials", success: false })
        }


        const data = {
            user: {
                id: user.id
            }
        }

        const authToken = jwt.sign(data, JWT_SECRET)                //  an authentication token is generated by joining the data with the secret(so that the user can be verified later) which is then sent as the response......
        res.json({ authToken: authToken, success: true })
    }

    catch (error) {
        console.error(error.message)
        res.status(500).json({ errors: "Internal Server Error", success: false })
    }
})




//  ROUTE 3 : Get loggedin user details using : POST "http://localhost:5000/api/auth/getuser"  Login required...

router.post('/getuser', fetchuser, async (req, res) => {

    try {

        const userId = req.user.id
        const user = await User.findById(userId).select("-password")                  //  all fields stored except password......

        res.send(user)
    }

    catch (error) {
        console.error(error.message)
        res.status(500).send("Internal Server Error")
    }
})




module.exports = router




